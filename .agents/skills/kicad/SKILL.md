...
- user / firmware configuration notes near the beginning of the report when the design has population options, sleep-state requirements, host-controlled rails, bring-up settings, mating-design information, or other end-user setup contracts
- MCU / GPIO internal pull configuration table near the beginning of the report when any MCU, SoC, or programmable device controls sleep-state or peripheral pins; prefer `project_config.mating_design.host_pin_map` when present
...
**Schematic** — verify: decoupling caps on every IC VCC/GND pair, I2C pull-ups, reset pin circuits, unconnected pins have no-connect markers, consistent net naming across sheets, ESD protection on external connectors, power sequencing (EN/PG), adequate bulk capacitance, and MCU GPIO sleep-state/internal pull requirements for every net that can float, wake the MCU, power-gate a peripheral, select an SPI device, clock a bus, or back-power an unpowered peripheral.
...
When producing a design review report, read `references/report-generation.md` for the standard report template, severity definitions, writing principles, and domain-specific focus areas. The report format covers: overview, critical findings, early user / firmware configuration notes, component summary, power tree, analyzer verification (spot-checks), signal/power/design analysis review, quality & manufacturing, prioritized issues table, positive findings, and known analyzer gaps. Put end-user and bring-up contracts near the top of the report: MCU internal pulls, sleep/reset GPIO states, optional or DNP pull-ups, solder/jumper choices, host-controlled rails, mating-design pin maps, I2C speed limits from weak pulls, documented power/current profiles, and similar configuration requirements should appear before detailed analyzer sections. When analyzer JSON includes `project_config.power_profile`, use it to calibrate current-capacity, via-current, thermal, sleep-current, and battery-life findings. When analyzer JSON includes `project_config.mating_design`, use its `notes`, `host_pin_map`, and `population_options` as the starting point for this section and preserve `TODO_*` placeholders as open items. Always cross-reference analyzer output against the raw schematic before reporting findings.
