...
### `mating_design` (object) — v1.3

Structured information about the board or harness that mates with the KiCad
project being analyzed. This is especially useful for breakout boards,
daughtercards, fixtures, interposers, and sensor boards where the host MCU or
power sequencing lives on another schematic.

Analyzers preserve this object in `project_config`. Report generation must read
it and surface relevant user/firmware information near the beginning of the
report, especially `host_pin_map` and `population_options`.

| Field | Type | Description |
|-------|------|-------------|
| `name` | string | Mating board, host board, fixture, or harness name |
| `host_mcu` | string | MCU/SoC/FPGA/radio module that controls the breakout |
| `source` | string | Where the mapping came from, e.g. "mating schematic", "user-provided", "firmware pin map" |
| `notes` | array of strings | Free-form design-contract notes to include in the early report notes |
| `host_pin_map` | array of objects | Per-header/per-net mapping from this board to the mating design |
| `population_options` | array of objects | DNP/jumper/solder-bridge/alternate-population choices that affect operation |

#### `mating_design.host_pin_map[]`

Each entry describes one breakout pin, connector pin, or named net whose behavior
depends on the mating design.

| Field | Type | Description |
|-------|------|-------------|
| `breakout_header` | string | Local connector pin such as `"J3.9"` |
| `breakout_net` | string | Local KiCad net name |
| `host_pin` | string | Host MCU pin, host rail, ground, or a `TODO_*` placeholder |
| `host_function` | string | Intended host-side function |
| `connected_part` | string | Local component/pin or subsystem attached to the net |
| `required_state` | string | Required host GPIO/supply/reset/sleep behavior |
| `reason` | string | Why the required state matters |

Use explicit placeholders such as `TODO_J4_8` or `TODO_LED1_HOST_PIN` when the
mating pin is not known yet. A placeholder is better than omission because it
keeps the report from silently dropping connector pins.

```jsonc
"mating_design": {
  "name": "TorporTag",
  "host_mcu": "STM32L432",
  "source": "User-provided mating-design intent",
  "notes": [
    "TMP119 power is supplied by STM32 PB0 through tmp119_pwr.",
    "TMP119 I2C pull-ups are normally STM32 internal pulls; I2C clock is slow."
  ],
  "host_pin_map": [
    {
      "breakout_header": "J3.9",
      "breakout_net": "tmp119_sda",
      "host_pin": "PA1",
      "host_function": "I2C SDA",
      "connected_part": "U3 TMP119 SDA, optional R5 to tmp119_pwr",
      "required_state": "Internal pull-up only when tmp119_pwr is on; no-pull/high-Z when off.",
      "reason": "Avoid back-powering TMP119 while using weak internal pull-up for slow I2C."
    },
    {
      "breakout_header": "J4.8",
      "breakout_net": "LED1",
      "host_pin": "TODO_LED1_HOST_PIN",
      "host_function": "LED1 active-low drive",
      "connected_part": "D1 cathode, R1 to +3.3V",
      "required_state": "Pull-up or push-pull high for LED off; drive low for LED on.",
      "reason": "LED is active-low from host perspective."
    }
  ],
  "population_options": [
    {
      "refs": ["R3", "R4", "R5"],
      "default": "DNP",
      "condition": "Populate only if STM32 internal pull-ups are insufficient.",
      "connected_rail": "tmp119_pwr",
      "notes": "Optional pull-ups are tied to the switched TMP119 rail."
    }
  ]
}
```
