...
- Critical Findings
- User / Firmware Configuration Notes, when the board has population options, sleep-state requirements, host-controlled rails, end-user setup steps, or firmware contracts
- MCU / GPIO Internal Pull Configuration, when an MCU or programmable SoC exists, placed inside the early User / Firmware Configuration Notes section
- Component Summary
- Power Tree
...
[**Include whenever the design has an MCU, SoC, FPGA, radio module with GPIOs, or any programmable part controlling peripheral pins.** Produce a table of pins whose sleep/reset/peripheral state depends on internal pull-ups, pull-downs, output drive, analog/no-pull, or explicit high-Z behavior. Prefer `project_config.mating_design.host_pin_map` when present, especially for breakout boards whose host MCU lives on a mating schematic. Otherwise build this from `ic_pin_analysis`, net names, connected peripheral pin types, and raw schematic review. Include at minimum: MCU pin, net, connected part/pin, required internal state, sleep/reset state, and why it matters. Cover I2C/SPI chip-selects, bus clocks, wake/interrupt lines, reset/boot/debug pins, power-gated rails, enable pins, unused GPIOs, and nets that can back-power unpowered peripherals. Downgrade intentional no-external-pull designs to firmware-contract notes only after confirming the designer intent; still state timing/leakage constraints such as slow I2C for weak internal pulls. Example columns: Pin | Net | Connected part | Internal state to use | Sleep/reset caveat | Reason.]

### Population / Option Notes
[Include DNP parts, jumper/solder-bridge options, alternate assembly choices, and any "populate only if..." decisions that affect function. Prefer `project_config.mating_design.population_options` when present, then cross-check against the schematic/BOM/CPL. State the default population clearly and explain what changes when the option is fitted. For optional pull-ups/pull-downs, name the nets, destination rail, default status, and firmware/hardware condition that would justify population.]
