...
def main():
    ...
    # Attach full project config to output for downstream consumers.
    # Report generation uses project_config.mating_design for breakout/host
    # pin maps, population options, and firmware bring-up contracts.
    if config:
        result["project_config"] = config
        if config.get("project"):
            result["project"] = config.get("project", {})
