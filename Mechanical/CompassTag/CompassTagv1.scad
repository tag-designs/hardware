...
version_string = "CTagV1"; // imprinted in base and lid
...
pogo_center = [6.45, 0.0];   // offset from board center of pogo pins
...
module makeLid(){
	translate([0,0,10])difference(){
        union(){
	     ...
	     #translate([-pcb_len/2+1.4,0,-end_height/2-1.8])
	         centerCube(end_length,end_width-1,end_height+1.8);
             // centered over the pogo pins
	     translate([pogo_center[0],pogo_center[1],-end_height/2+0.5])
	         centerCube(end_length,end_width,end_height-1);
             // body
