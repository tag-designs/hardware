...
pcb_clearance = 0.001*25.4;             // edge clearance (routing error)
pcb_len       = 21.550 + pcb_clearance*2;
pcb_width     = 11.5 + pcb_clearance*2;
    // battery support lip, length is arbitrary since this is just
    // to punch a hole
lip_len   = 0;                    
lip_width = 0; //4 + pcb_clearance*2;
lip_rad   = 0;//0.5 + pcb_clearance;

cutclear = 1.5; // horizontal space around cutout under pcb (/2 on each edge) 
pogo_center = [8.175, 0.0];   // offset from board center of pogo pins
...
base_height      = pogo_pin_height + 1;  // base holds 1.5mm board
...
align_pin_off    = [-8.5,10];             // alignment pin offset
...
module makeCrossBarBase(height,endheight,hole) {
   translate([pogo_center[0]+post_off, pogo_center[1],0])
   
             union(){
                   // cross piece
                translate([-2.5,0,0])centerCube(2*post_rad+5,post_dist,height);
                   // end cylinders
                translate([0,post_dist/2,0])
		            cylinder(r=post_rad, h=endheight);
                translate([-3.5,post_dist/2,0])
		            cylinder(r=post_rad, h=endheight);
                translate([-1.75,post_dist/2,0])
                    centerCube(3.5,6,endheight);
                translate([0,-post_dist/2,0])
		            cylinder(r=post_rad, h=endheight);
                translate([-3.5,-post_dist/2,0])
		            cylinder(r=post_rad, h=endheight);
        ...
        translate([0,0,-pogo_pin_height])
	        union(){
                centerCube(base_len,base_width,base_height);
 	            makeCrossBarBase(base_height,base_height,post_hole_insert);
            };
        ...
        union(){
            ...
            #translate([pogo_center[0]+align_pin_off[0],
  	        ...
  	        translate([pogo_center[0]+align_pin_off[0],
                 ...
                 // post holes
             translate([pogo_center[0]+post_off,pogo_center[1],0])
                 translate([0,post_dist/2,-6])
                    cylinder(r=post_hole_insert,h=(base_height+2));
                translate([pogo_center[0]+post_off,pogo_center[1],0])
                 translate([0,-post_dist/2,-6])
                    cylinder(r=post_hole_insert,h=(base_height+2));
           ...
           translate([8,4.5,pcb_thick/2-0.4])
             ...
             // thin battery end
  // battery
      //   #translate([pcb_len/2.0+2.5,0,pcb_min_thick])cylinder(r=6,h=pcb_thick);
	  //      translate([pcb_len/2 - 1,-7/2,pcb_min_thick])
	  //       cube([5,7,pcb_thick]);
...
module makeLid(){
	translate([0,0,10])difference(){
        union(){
	     ...
	     #translate([-pcb_len/2+2,0,-end_height/2-.1])
	         centerCube(end_length,end_width-1,end_height+.1);
             // centered over the pogo pins
	     translate([pogo_center[0],pogo_center[1],-end_height/2-1.8])
	         centerCube(end_length,end_width,end_height+1.8);
             // body
